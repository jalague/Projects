...
def followLeftWall(maze,t,col):     #this function actually solves the maze
    ...
    while i!=(len(maze)-1) or j!=(len(maze[1])): # this will have the turtle continue to 'solve' the maze until it reaches the exit
        currDirection=nextMove(maze,i,j,currDirection)   #this will find the next direction which the turtle will go
        
        if currDirection=="north":          #these conditionals simply change the values of i and j based on which direction the turtle is heading
            i=i-1
            ...
            j=j-1
        drawSquare(-150+1*(j*10),150-(i*10),t,10,col)   #a square is now drawn in the direction given by next move
        


def main(mazefile):    #this is the main function that will be called to both draw and solve the maze
    maze=readMaze(mazefile)   #here the list returned by the readMaze fucntion is assigned to the variable maze
    import turtle 
    scr=turtle.Screen()  #here I set up turtle graphics
    bob=turtle.Turtle()
    scr.tracer(2)             
    drawMaze(maze,bob,"black")  #this calls the drawMaze function with the turtle bob and makes the maze black
    
    followLeftWall(maze,bob,"yellow")   #this will call the solver, yellow, to solve the the just drawn maze
    scr.update()
    


main("testMaze2.txt")   #here I test one of the mazes
