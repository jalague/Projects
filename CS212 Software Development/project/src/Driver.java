...
public class Driver {
	...
	public static void main(String[] args) {
		...
		if ((parser.getDirectoryString() != null) && Files.isDirectory(parser.getDirectoryPath())) {

			Traverse search = new Traverse(parser.getDirectoryPath());
			TreeSet<Path> fileList = search.getFiles();
			InvertedIndex index = new InvertedIndex(parser.getIndexPath());
			index.convertAll(fileList);
			if (parser.getIndexString() != null) {

				index.outputJson();

			}
		} else {
			System.out.println("Please enter a valid directory with appropriate flagging");
		}
